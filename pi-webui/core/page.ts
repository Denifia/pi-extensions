...
export function renderWebUiPage(sessionData: unknown, themeName?: string, authToken?: string): string {
	const assets = loadCoreExportHtmlAssets();
	const sessionDataBase64 = Buffer.from(JSON.stringify(sessionData)).toString("base64");
	const css = renderCoreCss(assets.templateCss, themeName);
	const bootstrapJs = `window.__PI_WEBUI_TOKEN__ = ${JSON.stringify(authToken ?? "")};`;
	const js = `${bootstrapJs}\n${assets.templateJs}\n\n${WEBUI_SHELL_JS}`;
